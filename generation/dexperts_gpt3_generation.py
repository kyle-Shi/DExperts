...
class DExpertsGPT3Generation(GPT2Generation):
    ...
    def get_gpt3_logits(self, input_ids, filter_p):
        ...
        response_logits = [
            choice["logprobs"]["top_logprobs"] for choice in response["choices"]
        ]

        gpt3_logits = -50000.0 * torch.ones(
            [len(prompts), 1, len(self.tokenizer)], dtype=torch.float32
        ).to(self.device)
        ...
        with torch.no_grad():
            for step in range(max_len):
                ...
                if step == 0:
                    ...
                    expert_next_token_logits = expert_logits[
                        range(batch_size), last_non_masked_idx, :
                    ]
                    antiexpert_next_token_logits = antiexpert_logits[
                        range(batch_size), last_non_masked_idx, :
                    ]
                else:
                ...
                next_token_logits = gpt3_next_token_logits + alpha * (
